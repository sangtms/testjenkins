pipeline {
    ...
    stages {
        stage('Build') {
            steps {			
				...
				bat 'python mattermost.py [STARTED] ' + env.BRANCH_NAME + ' ' + env.JOB_NAME + ' #' + env.BUILD_NUMBER + ' (Changes) (' + env.COMMIT_MESSAGE + ') [' + env.BLUE_OCEAN_URL +'](View detail)'
    ...
    post {
        ...
        success {
			...
			bat 'python mattermost.py [SUCCESS] ' + env.JOB_NAME + ' #' + env.BUILD_NUMBER + ' after ' + env.BUILD_DURATION + '[' + env.BLUE_OCEAN_URL + '](View detail)'
        }
        failure {
			//bat 'python mattermost.py [FAILED] ${JOB_NAME} #${BUILD_NUMBER} after ${BUILD_DURATION} (<a href="${BLUE_OCEAN_URL}">View detail</a>)'
			bat 'python mattermost.py [FAILED] ' + env.JOB_NAME + ' #' + env.BUILD_NUMBER + ' after ' + env.BUILD_DURATION + '[' + env.BLUE_OCEAN_URL + '](View detail)'
        }
    }
}