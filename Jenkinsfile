    ...
    stages {
        stage('Build') {
			...
			//echo bat(returnStdout: true, script: 'set')
				...
				bat 'python mattermost.py [STARTED] ' + env.BRANCH_NAME + ' ' + env.JOB_NAME + ' #' + env.BUILD_NUMBER + ' (' + env.HIPCHAT_CHANGES_OR_CAUSE+ ') (' + bat(returnStdout: true, script: "git log -n 1 --pretty=format:'%h'").trim() + ') [View detail](' + env.BLUE_OCEAN_URL + ')'
