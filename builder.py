...
class MsBuilder:
	...
	def build_net_core(self, projPath):
		p = subprocess.call(['dotnet', 'build', projPath, '--configuration','release'])
		if p==0: return True	# exit early
		
		return False

	def deploy_net_core(self, projPath, output_relative_path):
		print "dotnet publish --configuration release %s --output %s" % (projPath, output_relative_path)
		p = subprocess.call(['dotnet', 'publish', '--configuration', 'release', projPath, '--output', output_relative_path])
		if p==0: return True	# exit early
...
def stop_app_pool(server_name, app_pool_name):
	...
	p = subprocess.Popen(["powershell.exe", stop_apppool_script], stdout=sys.stdout)
	p.communicate()
	if p==1:
		raise Exception("Have problem when stop_app_pool!")
...
def start_app_pool(server_name, app_pool_name):
	...
	p = subprocess.Popen(["powershell.exe", start_apppool_script], stdout=sys.stdout)
	p.communicate()
	if p==1:
		raise Exception("Have problem when start_app_pool!")